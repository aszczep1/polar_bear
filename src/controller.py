...
class Controller:
    ...
    def menuloop(self):
        ...
        self.menu = pygame_menu.Menu('Cocaine Bear', 800, 600,theme=pygame_menu.themes.THEME_BLUE)
        self.menu.add.text_input('Welcome to Cocaine Bear', cursor_size = (1,1) )
        self.menu.add.text_input('Collect 10 bags of coke to win')
        self.menu.add.text_input('Careful, avoid the hunters. They have reach.')
        self.menu.add.button('Play', lambda: self.gameloop(level))
    ...
    def gameloop(self, level):
        '''
         ...
         score is greater than 1000. It then switches to either the loss loop or win
        ...
        # Handle events
            ...
            level_points = 10 * level
            if score >= level_points:
                self.state = "WIN"
                level += 1
                self.winloop(level)
    ...
    def lossloop(self):
        ...
        self.menu = pygame_menu.Menu('Cocaine Bear', 500, 500,theme=pygame_menu.themes.THEME_BLUE)
        level = 1
        self.menu.add.button('Try Again', lambda: self.gameloop(level))
        self.menu.add.button('Quit', pygame_menu.events.EXIT)
        self.menu.mainloop(window)
    
    def winloop(self, level):
        '''
        ...
        pygame.time.wait(4000)
        window = pygame.display.set_mode((800, 800))
        score_needed = 10 * level 
        self.menu = pygame_menu.Menu(f'Cocaine Bear: Level {level} ', 500, 500,theme=pygame_menu.themes.THEME_BLUE)
        self.menu.add.text_input(f'{score_needed} points needed  to win', cursor_size= (1, 1))
        self.menu.add.button(f'Play level {level}', lambda: self.gameloop(level))
        self.menu.add.button('Quit', pygame_menu.events.EXIT)
        self.menu.mainloop(window)
